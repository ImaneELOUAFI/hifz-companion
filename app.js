...
let allVerses = []; let quizVerses = []; let examQuestions = []; 
...
const srsCheckbox = document.getElementById('srs-checkbox'); 
...
async function loadSurahNames() {
    try {
        surahSelect.innerHTML = `<option>${t('loadingVerses')}</option>`;
...
async function initApp() {
    ...
    try {
        ...
        document.getElementById('stats-toggle').addEventListener('click', () => {
            homeScreen.style.display = 'none'; streakInfoScreen.style.display = 'none'; statsScreen.style.display = 'block'; renderStatsDisplay();
        });
        ...
        document.getElementById('reset-stats-btn').addEventListener('click', () => {
            if(confirm(t('resetStats') + " ?")) {
                userStats = { totalAnswered: 0, correctFirstTry: 0, sessionsCompleted: 0, currentStreak: 0, lastStudyDate: null };
                userMistakes = {};
        ...
        document.getElementById('start-btn').addEventListener('click', () => {
            ...
            if (isExamMode) {
                document.getElementById('quiz-title').innerText = t('examModeLabel'); quranPageDiv.style.display = 'none'; examPromptBox.style.display = 'block'; loadExamVerses(reciterSelect.value);
            } else {
                const surahName = surahSelect.options[surahSelect.selectedIndex].text.split('(')[0];
                const revTxt = currentLang === 'ar' ? 'مراجعة' : (currentLang === 'en' ? 'Review' : 'Révision');
                document.getElementById('quiz-title').innerText = `${revTxt} : ${surahName}`;
                quranPageDiv.style.display = 'block'; examPromptBox.style.display = 'none'; quranPageDiv.innerHTML = ""; loadSurahVerses(surahSelect.value, reciterSelect.value);
            }
        });
...
function updateFavButtonStatus() {
    const selectedId = String(reciterSelect.value);
    if (favoriteReciters.includes(selectedId)) { 
        favBtn.innerText = t('removeFav'); 
        favBtn.classList.add('is-favorite');
    } else { 
        favBtn.innerText = t('addFav'); 
        favBtn.classList.remove('is-favorite');
    }
}
...
function updateProgress(total) {
    if(total === 0) return;
    const percentage = Math.round((currentVerseIndex / total) * 100);
    progressBar.style.width = `${percentage}%`; progressText.innerText = `${currentVerseIndex} / ${total}`; progressPercentage.innerText = `${percentage}%`;
}
...
function getAudioUrl(correctVerse, selectedReciterId) {
    if (selectedReciterId.startsWith('ea_')) {
        ...
        const parts = correctVerse.verse_key.split(':'); const surahStr = parts[0].padStart(3, '0'); const ayahStr = parts[1].padStart(3, '0');
...
async function loadSurahVerses(chapterId, reciterId) {
    ...
    currentVerseIndex = 0; score = 0; madeMistake = false; allVerses = []; quizVerses = []; updateProgress(1);
    ...
    try {
        ...
        quizVerses = allVerses.slice(startV - 1, endV); generateQuestion();
    } catch (error) { console.error(error); }
}

async function loadExamVerses(reciterId) {
    optionsContainer.innerHTML = ""; examPromptBox.innerHTML = ""; feedbackEl.innerText = "";
    if (currentAudio) { currentAudio.pause(); currentAudio = null; }
    currentVerseIndex = 0; score = 0; madeMistake = false; allVerses = []; examQuestions = []; updateProgress(1);
    ...
    try {
        ...
        let selectedIndices = [];
        const useSRS = srsCheckbox.checked;

        if (useSRS) {
            let pool = [];
            for (let i = 0; i < allVerses.length - 1; i++) {
                let vKey = allVerses[i].verse_key;
            ...
            for (let idx of pool) {
                if (!selectedIndices.includes(idx)) {
                    ...
                    if (selectedIndices.length === numQ) break;
                }
            }
        } else {
            let possibleIndices = []; for(let i=0; i<allVerses.length - 1; i++) possibleIndices.push(i);
            possibleIndices.sort(() => Math.random() - 0.5); 
            selectedIndices = possibleIndices.slice(0, numQ);
        }
        
        for(let i=0; i<numQ; i++) {
            let promptIndex = selectedIndices[i];
            examQuestions.push({ promptVerse: allVerses[promptIndex], correctNextVerse: allVerses[promptIndex + 1] });
        }
        generateQuestion();
    } catch (error) { console.error(error); }
}
...
function generateQuestion() {
    feedbackEl.innerText = ""; optionsContainer.innerHTML = ""; madeMistake = false; 
    replayAudioBtn.style.display = 'none';

    const totalQuestions = isExamMode ? examQuestions.length : quizVerses.length;
    ...
    if (currentVerseIndex >= totalQuestions) {
        ...
        instructionEl.innerText = t('congrats'); examPromptBox.style.display = 'none';
        ...
        optionsContainer.innerHTML = `
            <div dir="${textDir}" style="text-align: center; padding: 20px; background-color: var(--bg-controls); border-radius: 10px; border: 1px solid var(--border-color);">
                ...
                <p style="color: var(--text-main);">${score} ${t('foundOnFirstTry')} ${totalQuestions}.</p>
    ...
    if (isExamMode) {
        ...
        const q = examQuestions[currentVerseIndex];
        let displayPromptText = cleanVerseText((useTajweed && q.promptVerse.text_uthmani_tajweed) ? q.promptVerse.text_uthmani_tajweed : q.promptVerse.text_uthmani); 
        examPromptBox.innerHTML = `${displayPromptText} <span style="font-family: 'Amiri', serif; color: var(--primary-color);">۝${toArabicNumber(q.promptVerse.verse_key.split(':')[1])}</span>`;
        correctVerse = q.correctNextVerse;
    } else {
    ...
    if (isExamMode) contextVerse = examQuestions[currentVerseIndex].promptVerse;
    ...
    let similarVerses = allVerses.filter(v => v.id !== correctVerse.id && v.text_uthmani.startsWith(firstWord));
    similarVerses.sort(() => Math.random() - 0.5);
    for (let verse of similarVerses) if (options.length < 4) options.push(verse);

    if (options.length < 4) {
        let lengthSimilarVerses = allVerses.filter(v => !options.some(opt => opt.id === v.id) && Math.abs(v.text_uthmani.split(' ').length - correctVerse.text_uthmani.split(' ').length) <= 2);
        lengthSimilarVerses.sort(() => Math.random() - 0.5);
        for (let verse of lengthSimilarVerses) if (options.length < 4) options.push(verse);
    }

    while (options.length < 4 && options.length < allVerses.length) {
        const randomIndex = Math.floor(Math.random() * allVerses.length);
        if (!options.some(opt => opt.id === allVerses[randomIndex].id)) options.push(allVerses[randomIndex]);
    }
...
function checkAnswer(selectedId, correctId, correctText, audioUrl, verseKey) {
    if (selectedId === correctId) {
        ...
        if (isExamMode) examPromptBox.innerHTML = `${cleanCorrectText} <span style="font-family: 'Amiri', serif; color: var(--primary-color);">۝${arabicVerseNum}</span>`;
        else {
